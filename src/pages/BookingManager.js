...
const BookingManager = () => {
    ...
    const fetchBookings = async () => {
        try {
            ...
            if (user.is_staff) {
                ...
                setBookings(res.data.filter(b => b.customer_id === user.user_id));
            }
        } catch (e) { setBookings([]); }
    };
    ...
    const handleCreate = async (e) => {
        ...
        } catch (err) { alert('Error: Check room availability'); }
    };
    ...
    return (
        <div className="container">
            ...
            <form className="mb-4 p-3 bg-light" onSubmit={handleCreate}>
                <div className="row">
                    {user.is_staff && (
                         <div className="col-md-3"><input className="form-control" placeholder="Customer ID" onChange={e => setNewBooking({...newBooking, customer_id: e.target.value})} /></div>
                    )}
                ...
                <button className="btn btn-primary mt-2">Create Booking</button>
            </form>
            <table className="table">
                <thead><tr><th>ID</th><th>Room</th><th>In</th><th>Out</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                    {bookings.map(b => (
                        <tr key={b.booking_id}>
                            ...
                            <td>
                                {b.status !== 'completed' && user.is_staff && 
                                    <button className="btn btn-sm btn-info" onClick={() => handleCheckout(b.booking_id)}>Checkout</button>
                                }
                            </td>
                        </tr>
                    ))}
