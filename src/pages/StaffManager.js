...
const StaffManager = () => {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Delete this staff?')) {
            try { await axiosClient.delete(`/staff/${id}`); fetchStaff(); } catch (err) { alert(err.message); }
        }
    };
    ...
    return (
        <div className="container">
            ...
            <form className="mb-4 row g-2" onSubmit={handleCreate}>
                ...
                <div className="col-auto"><button className="btn btn-primary">Add Staff</button></div>
            </form>
